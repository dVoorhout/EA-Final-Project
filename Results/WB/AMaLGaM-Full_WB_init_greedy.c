...
short check_if_points_within_radius(double radius, point points[], point a, point b);
point findMiddlePoint(point a, point b);
...
void spherePackingFunctionProblemEvaluation(double *parameters, double *objective_value, double *constraint_value);
double spherePackingFunctionProblemLowerRangeBound(int dimension);
double spherePackingFunctionProblemUpperRangeBound(int dimension);
...
void interpretCommandLine(int argc, char **argv) {
	...
	if (use_guidelines) {
		tau = 0.35;
		population_size = (int)(17.0 + pop_size_multiplier * pow((double)number_of_parameters, 1.5));
...
void parseParameters(int argc, char **argv, int *index) {
	...
	noError = noError && sscanf(argv[*index + 15], "%d", &check_local_optimum_count);
	noError = noError && sscanf(argv[*index + 16], "%lf", &pop_size_multiplier);
...
void printVerboseOverview(void) {
	...
	printf("# Local optimum threshold = %d\n", check_local_optimum_count);
...
char *installedProblemName(int index) {
	...
	case 13: return((char *) "SpherePacking");
	}
...
double installedProblemLowerRangeBound(int index, int dimension) {
	...
	case 13: return(spherePackingFunctionProblemLowerRangeBound(dimension));
	}
...
double installedProblemUpperRangeBound(int index, int dimension) {
	...
	case 13: return(spherePackingFunctionProblemUpperRangeBound(dimension));
	}
...
void installedProblemEvaluationWithoutRotation(int index, double *parameters, double *objective_value, double *constraint_value) {
	...
	*constraint_value = 0.0;
	...
	case 13: spherePackingFunctionProblemEvaluation(parameters, objective_value, constraint_value); break;
	}
}
...
//double find_smallest_distance_between_points(point points[], int number_of_points) {
//	double smallestDistanceFound = 1000;
//	double calculatedDistance = 0;
//	point smallestA, smallestB;
//	int indexA, indexB;
...
short check_if_points_within_radius(double radius, point points[], point a, point b) {
	...
	for (int i = 0; i < number_of_parameters / 2; i++) {
		if ((euclidianDistance(points[i], a) <= radius || euclidianDistance(points[i], b) <= radius) && (fabs(points[i].x - a.x) > check || fabs(points[i].y - a.y) > check) && (fabs(points[i].x - b.x) > check || fabs(points[i].y - b.y) > check)) {
			return 1;
		}
	}
	return 0;
...
void spherePackingFunctionProblemEvaluation(double *parameters, double *objective_value, double *constraint_value) {
	...
	int    i;
	double result;
	int numberOfPoints = number_of_parameters / 2;
	...
	for (i = 0; i < number_of_parameters; i++) {
		if (i % 2 == 0)
			points[(int)floor(i / 2)].x = parameters[i];
		else {
			points[(int)floor(i / 2)].y = parameters[i];
		}
	}
	...
	//printf("Smallest distance found: %lf |A(%lf, %lf), B(%lf, %lf)\n", smallestDistanceFound, smallestA.x, smallestA.y, smallestB.x, smallestB.y);
	if (numberOfPoints > 2) {
		point middlePoint = findMiddlePoint(smallestA, smallestB);
		double radius = smallestDistanceFound;
		//printf("Radius we have: %lf\n", radius);
		double search_multiplier = 0.01;
		short terminated = 0;
		while (!terminated) {
			radius += smallestDistanceFound * search_multiplier;
			if (check_if_points_within_radius(radius, points, smallestA, smallestB) || radius > 1.5) {
				terminated = 1;
			}
		}

		//printf("Radius we can expand to: %lf\n", radius);

		double distance_to_move_points = (radius - smallestDistanceFound) / 4;
		//printf("Moving the points by %lf\n", distance_to_move_points);
		/*point new_point_A = { .x = smallestA.x + distance_to_move_points ,.y = smallestA.y + distance_to_move_points };
		point new_point_B = { .x = smallestB.x - distance_to_move_points,.y = smallestB.y - distance_to_move_points };
		if (euclidianDistance(new_point_A, smallestB) < smallestDistanceFound) {
			point new_point_A = { .x = smallestA.x - distance_to_move_points,.y = smallestA.y - distance_to_move_points };
			point new_point_B = { .x = smallestB.x + distance_to_move_points,.y = smallestB.y + distance_to_move_points };
		}*/

		double vector_x, vector_y;
		vector_x = smallestA.x - smallestB.x;
		vector_y = smallestA.y - smallestB.y;
		double growth_factor = (smallestDistanceFound + distance_to_move_points) / smallestDistanceFound;
		/*printf("Growthfactor: %lf", growth_factor);*/
		point new_point_A = { .x = smallestB.x + vector_x * growth_factor,.y = smallestB.y + vector_y * growth_factor };
		point new_point_B = { .x = smallestA.x - vector_x * growth_factor,.y = smallestA.y - vector_y * growth_factor };

		parameters[indexA * 2] = new_point_A.x;
		parameters[indexA * 2 + 1] = new_point_A.y;
		parameters[indexB * 2] = new_point_B.x;
		parameters[indexB * 2 + 1] = new_point_B.y;
	...
	double newSmallestDistanceFound = 1000;
	calculatedDistance = 0;
	for (int i = 0; i < numberOfPoints - 1; i++) {
		for (int j = i + 1; j < numberOfPoints; j++) {
			calculatedDistance = euclidianDistance(points[i], points[j]);
			if (calculatedDistance < newSmallestDistanceFound) {
				newSmallestDistanceFound = calculatedDistance;
			}
		}
	}

	//printf("new best distance: %lf\n", newSmallestDistanceFound);
	*objective_value = 1 / newSmallestDistanceFound;
	*constraint_value = 0;
}
...
void initializeParameterRangeBounds(void) {
	...
	short found_square = 0;
	int square = 1;
	int largest_square = 0;
	while (!found_square) {
		if (pow(square, 2) > number_of_parameters / 2) {
			largest_square = square - 1;
			found_square = 1;
		}
		square += 1;
	}
	...
	printf("%d\n%", largest_square);
	double subsection_length = 1 / (double)largest_square;

	int counter = 0;
	for (int i = 0; i < largest_square; i++) {
		for (int j = 0; j < largest_square; j++) {
			//x
			lower_range_bounds[counter * 2] = i * subsection_length;
			upper_range_bounds[counter * 2] = i * subsection_length + subsection_length;
			//y
			lower_range_bounds[counter * 2 + 1] = j * subsection_length;
			upper_range_bounds[counter * 2 + 1] = j * subsection_length + subsection_length;
	...
	for (int i = 0; i < number_of_parameters; i++) {
		printf("%d %lf %lf\n", i, lower_range_bounds[i], upper_range_bounds[i]);
	}
...
short checkVTRTerminationCondition(void) {
	...
	printf("%d ", number_of_generations);
	double steps_needed_to_reach_vtr = (double)result / vtr;
	printf("%lf\n", steps_needed_to_reach_vtr);

	if (number_of_generations == (100 - check_local_optimum_count)) {
		latest_local_optimum_value = steps_needed_to_reach_vtr;
	}

	if (number_of_generations >= 100 && number_of_generations % check_local_optimum_count == 0) {
		if (fabs(steps_needed_to_reach_vtr - latest_local_optimum_value) < (double)(0.10 * latest_local_optimum_value)) {
			return 1;
		}
		latest_local_optimum_value = steps_needed_to_reach_vtr;
	}
...
short checkFitnessVarianceTerminationSinglePopulation(int population_index) {
	...
	if (objective_var <= fitness_variance_tolerance) {
		printf("----------Terminated because of Variance--------");
		return(1);
	}
	...
	for (i = 0; i < number_of_populations; i++) {
		if (!populations_terminated[i])
			if (distribution_multipliers[i] < 1e-10) {
				printf("-----------Terminated because of dist multi-----------");
				populations_terminated[i] = 1;
			}
	}
}
